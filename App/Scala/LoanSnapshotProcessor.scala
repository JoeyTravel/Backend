...
object  SalesforceLoanSnapshotProcessor {
  def asynchronously(
    ...
    val workQueue = AsynchronousWorkQueueFactory(statsReceiver)
...
/**
  ...
  * archiveLibrary for [[com.expeditelabs.thrift.events.LendingQbSnapshot]]s. This snapshots
...
class SalesforceLoanSnapshotProcessor(
  ...
  override def isDefinedAt(message: events.EventMessage): Boolean = {
    message.event == Events.lendingQbBackup && message.topic == Topics.lendingQbBackup
  }
